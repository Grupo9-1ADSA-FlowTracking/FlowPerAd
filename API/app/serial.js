...
class ArduinoDataRead {
  ...
  fake_data() {
    setInterval(() => {
      ...
      console.log(
        "temp: ",
        parseFloat(data_float[1].toFixed(2)),
        "hum: ",
        data_float[0],
        "chave: ",
        data_int,
        "lux: ",
        parseFloat(data_lux.toFixed(2))
      );
    ...
    SerialPort.list()
      ...
      .then((arduinoCom) => {
        ...
        parser.on("data", (data) => {
          ...
          let humidity = parseFloat(value[0]);
          let temperature = parseFloat(value[1]);
          let lux = parseFloat(value[2]);
          let temp2 = parseFloat(value[3]);
          let chave = parseFloat(value[4]);
